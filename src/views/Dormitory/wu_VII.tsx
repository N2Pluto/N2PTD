...
const DormitorydetailsVII = () => {
   const [dormitoryBuilding, setDormitoryBuilding] = useState([])
   useEffect(() => {
     const fetchData = async () => {
       try {
         const { data, error } = await supabase.from('Dormitory_Building').select('name , images_url').eq('dorm_id', 6)
         if (error) {
           throw error
         }
...
export default DormitorydetailsVII
